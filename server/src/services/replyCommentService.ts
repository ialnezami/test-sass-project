...
import { getReplyCommentRepository } from '../../db/repositories/index.js';
import { WORKSPACE_ROLES } from '../../../shared/types.js';

/**
 * Service de gestion des réponses aux commentaires
 * ✅ Conforme au pattern Agentova
 */

/**
 * Récupérer toutes les réponses aux commentaires d'un workspace
 */
export const getReplyComments = onCall({
  region: 'us-central1',
  memory: '512MiB',
  timeoutSeconds: 60
}, async (request) => {
  try {
    ...
    const replyComments = await getReplyCommentRepository().getByWorkspace(workspace_id);

    // ✅ 6. Logging succès structuré
    logger.info('Réponses aux commentaires récupérées avec succès', {
      workspace_id,
      user_id: uid,
      count: replyComments.length,
      action: 'get_reply_comments'
    });

    // ✅ 7. Réponse standardisée
    return response.success({ replyComments });
    
  } catch (error) {
    logger.error('Erreur dans getReplyComments', {
