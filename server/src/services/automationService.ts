...
import { getAutomationRepository } from '../../db/repositories/index.js';
import { WORKSPACE_ROLES } from '../../../shared/types.js';

/**
 * Service de gestion des automatisations
 * ✅ Conforme au pattern Agentova
 */

/**
 * Récupérer toutes les automatisations d'un workspace
 */
export const getAutomations = onCall({
  region: 'us-central1',
  memory: '512MiB',
  timeoutSeconds: 60
}, async (request) => {
  try {
    ...
    const automations = await getAutomationRepository().getByWorkspace(workspace_id);

    // ✅ 6. Logging succès structuré
    logger.info('Automatisations récupérées avec succès', {
      workspace_id,
      user_id: uid,
    ...
    return response.success({ automations });
    
  } catch (error) {
    logger.error('Erreur dans getAutomations', {
